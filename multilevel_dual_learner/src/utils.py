...
def plot_dual_learner_evaluation( evaluation_steps, biased_success_rates, unbiased_success_rates, filename="img/dual_learner_evaluation.png", ):
    ...
    unbiased = np.asarray(unbiased_success_rates, dtype=np.float64)
    if steps.ndim == 1:
        steps = steps[np.newaxis, :]
    if biased.ndim == 1:
        biased = biased[np.newaxis, :]
    if unbiased.ndim == 1:
        unbiased = unbiased[np.newaxis, :]
    if biased.shape != unbiased.shape or biased.shape != steps.shape:
        raise ValueError("evaluation arrays must share shape (num_seeds, eval_points)")
    if biased.size == 0:
        raise ValueError("evaluation arrays cannot be empty")
    if not np.all(steps == steps[0]):
        raise ValueError("evaluation checkpoints must match across seeds")

    _prepare_plot_path(filename)
    fig, ax = plt.subplots(figsize=(7.2, 4.4), constrained_layout=True)
    x_axis = steps[0]
    for values, color, label in (
        (biased, LEARNING_REWARD_COLOR, "Biased policy"),
        (unbiased, TASK_REWARD_COLOR, "Unbiased policy"),
    ):
    ...
    _place_legend_above(fig, handles, labels, max_columns=2)
    fig.savefig(filename, dpi=300, bbox_inches="tight")
